...
using SignalROrleansPlay;

var loggerFactory = new LoggerFactory();
// Creates 10 silos
var silos = await CreateSilos(10);
// Creates a signalR hub for each silo
var managers = silos.Select(silo => new OrleansHubLifetimeManager<MyHub>(silo.Client, loggerFactory.CreateLogger<OrleansHubLifetimeManager<MyHub>>())).ToArray();
// Connects 1000 clients to each signalR Hub
...
async Task<(IHost Host, IClusterClient Client)> CreateSilo(IPEndPoint? primarySiloEndpoint)
{
  var host = new HostBuilder()
    .UseOrleans(siloBuilder =>
    {
      ...
      siloBuilder.AddMemoryStreams("OrleansSignalR");
  ...
  var clientHost = new HostBuilder()
    .UseOrleansClient(clientBuilder =>
    {
      ...
      clientBuilder.AddMemoryStreams("OrleansSignalR");
    })
    .Build();
